...
namespace carFixMgr0611.ui
{
    partial class ReceiptView : MaterialForm //여기서 퍼블릭 뺏다
    {
        ...
        private void vSearchItem_Click(object sender, EventArgs e)
        {
            ...
            if (item.Equals("") || item.Equals("검색항목 선택") || value.Equals(""))
            {
                MessageBox.Show("정호가한 검색어를 입력하세요");
                return;
            }
            else
            {
            ...
            List<ReceiptVO> list = adapter.getReceiptDbBySearch(item, value);
            viewList.Items.Clear();
            initList(list);
        ...
        private void vSearchAll_Click(object sender, EventArgs e)
        {
            viewList.Items.Clear();
            initList(receiptList);
            viewSelect.SelectedIndex = -1; // 콤보박스 초기화, 맨위에것 선택할려면 1
